...
export default function Flashcards() {
  ...
  const [filterStatus, setFilterStatus] = useState<string>("all"); // all, learning, unknown
  
  const [deck, setDeck] = useState<Word[]>([...flashcardWords]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isFlipped, setIsFlipped] = useState(false);

  // Apply filters
  React.useEffect(() => {
    let filtered = [...flashcardWords];
    ...
    if (filterStatus === "learning") {
      ...
      filtered = filtered.filter(w => !knownWords[w.id]); // Anything not strictly known
    }

    setDeck(filtered);
    setCurrentIndex(0);
    setIsFlipped(false);
  }, [filterType, filterStatus, knownWords, learningWords]);
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6 py-6 px-4 md:py-10 flex flex-col h-[calc(100vh-80px)] md:h-auto">
      ...
      {deck.length === 0 ? (
        ...
        <div className="flex-1 flex flex-col w-full relative perspective-1000">
          ...
          {/* Flashcard Area */}
          <div 
            className="flex-1 w-full relative cursor-pointer group"
              ...
              >
                {!isFlipped ? (
                  ...
                  <div className="w-full h-full bg-card border-2 border-card-border rounded-3xl p-8 flex flex-col items-center justify-center text-center shadow-md relative overflow-hidden group-hover:border-primary/30 transition-colors">
                    ...
                    <h2 className="text-6xl md:text-8xl font-jp font-bold text-foreground mb-6">
                      {currentWord.kanji}
                    </h2>
          ...
          <div className={`mt-6 grid grid-cols-2 gap-4 shrink-0 transition-opacity duration-300 ${isFlipped ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <button 
              ...
              className="flex items-center justify-center gap-2 py-4 px-6 bg-card border-2 border-orange-200 text-orange-600 rounded-2xl font-bold text-lg hover:bg-orange-50 active:scale-95 transition-all"
            >
              <Clock className="w-6 h-6" /> Learning
            </button>
            <button 
              onClick={(e) => { e.stopPropagation(); handleKnown(); }}
              className="flex items-center justify-center gap-2 py-4 px-6 bg-success/10 border-2 border-success text-success-foreground rounded-2xl font-bold text-lg hover:bg-success/20 active:scale-95 transition-all"
            >
