...
import { getTeForm } from "@/lib/te-form";
import { useProgress } from "@/hooks/use-progress";
import { Zap, CheckCircle2, XCircle, ArrowRight, RotateCcw, Trophy, Info } from "lucide-react";
...
export default function QuizTeForm() {
  ...
  const verbPool = allWords.filter(w =>
    w.type.includes('verb') && w.verbGroup && getTeForm(w) !== null
  );
  ...
  const [questions, setQuestions] = useState<TeQuestion[]>([]);
  ...
  const startQuiz = () => {
    ...
    const allTeForms = verbPool
      .map(w => getTeForm(w))
      .filter(Boolean) as { kana: string; romaji: string; rule: string }[];

    const qs: TeQuestion[] = picked.map(word => {
      const tf = getTeForm(word)!;
      const wrongKanas = fisherYates(
        allTeForms.filter(t => t.kana !== tf.kana)
      )
        .slice(0, 3)
        .map(t => t.kana);
      const options = fisherYates([...wrongKanas, tf.kana]);
      return {
        word,
        correct: tf.kana,
        correctRomaji: tf.romaji,
        rule: tf.rule,
        options,
      };
    });
  ...
  if (!isStarted) {
    ...
    return (
      <div className="w-full max-w-lg mx-auto py-12 px-4 flex flex-col items-center justify-center min-h-[70vh] text-center space-y-8">
        <div className="w-24 h-24 bg-secondary/20 text-secondary-foreground rounded-full flex items-center justify-center mb-4">
          <Zap className="w-12 h-12 text-primary" />
        </div>

        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2">て-Form Quiz</h1>
          <p className="text-muted-foreground text-lg">Practice verb て-form conjugation.</p>
        </div>

        <div className="w-full bg-muted/40 border border-border rounded-2xl p-4 text-left space-y-1.5">
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 flex items-center gap-1"><Info className="w-3.5 h-3.5" /> Rules reminder</p>
          {[
            ['う/つ/る (godan)', '→ って', 'au → atte, matsu → matte'],
            ['む/ぶ/ぬ (godan)', '→ んで', 'yomu → yonde'],
            ['く (godan)', '→ いて', 'kaku → kaite'],
            ['ぐ (godan)', '→ いで', 'oyogu → oyoide'],
            ['す (godan)', '→ して', 'hanasu → hanashite'],
            ['る-verb (ichidan)', '→ て', 'taberu → tabete'],
            ['する / compound', '→ して', 'benkyousuru → benkyoushite'],
            ['くる', '→ きて', 'kuru → kite'],
          ].map(([pattern, result, ex]) => (
            <div key={pattern} className="flex items-center gap-2 text-xs">
              <span className="font-jp font-bold text-foreground w-32 shrink-0">{pattern}</span>
              <span className="text-primary font-bold w-14 shrink-0">{result}</span>
            ...
            {currentQ.options.map((option, i) => {
              ...
              return (
                <motion.button
                  ...
                  className={`p-4 rounded-2xl font-jp text-2xl font-bold transition-all duration-200 flex justify-between items-center ${btnClass}`}
                >
                  <span>{option}</span>
