...
import { getMasuForm } from "@/lib/te-form";
import { useProgress } from "@/hooks/use-progress";
import { Star, CheckCircle2, XCircle, ArrowRight, RotateCcw, Trophy, Info } from "lucide-react";
...
export default function QuizMasuForm() {
  ...
  const verbPool = allWords.filter(w =>
    w.type.includes('verb') && w.verbGroup && getMasuForm(w) !== null
  );
  ...
  const [questions, setQuestions] = useState<MasuQuestion[]>([]);
  ...
  const startQuiz = () => {
    ...
    const allMasuForms = verbPool
      .map(w => getMasuForm(w))
      .filter(Boolean) as { kana: string; romaji: string; rule: string }[];

    const qs: MasuQuestion[] = picked.map(word => {
      const mf = getMasuForm(word)!;
      const wrongKanas = fisherYates(
        allMasuForms.filter(t => t.kana !== mf.kana)
      )
        .slice(0, 3)
        .map(t => t.kana);
      const options = fisherYates([...wrongKanas, mf.kana]);
      return {
        word,
        correct: mf.kana,
        correctRomaji: mf.romaji,
        rule: mf.rule,
        options,
      };
    });
    ...
    return (
      <div className="w-full max-w-lg mx-auto py-12 px-4 flex flex-col items-center justify-center min-h-[70vh] text-center space-y-8">
        ...
        <div className="w-full bg-muted/40 border border-border rounded-2xl p-4 text-left space-y-1.5">
          ...
          {[
            ['う (godan)',       '→ います',   'au → aimasu, kau → kaimasu'],
            ['つ (godan)',       '→ ちます',   'matsu → machimasu'],
            ['る (godan)',       '→ ります',   'uru → urimasu, noru → norimasu'],
            ['む (godan)',       '→ みます',   'yomu → yomimasu'],
            ['ぶ (godan)',       '→ びます',   'asobu → asobimasu'],
            ['ぬ (godan)',       '→ にます',   'shinu → shinimasu'],
            ['く (godan)',       '→ きます',   'kaku → kakimasu'],
            ['ぐ (godan)',       '→ ぎます',   'oyogu → oyogimasu'],
            ['す (godan)',       '→ します',   'hanasu → hanashimasu'],
            ['る-verb (ichidan)', '→ ます',    'taberu → tabemasu, miru → mimasu'],
            ['する / compound',  '→ します',   'benkyousuru → benkyoushimasu'],
            ['くる',             '→ きます',   'kuru → kimasu'],
