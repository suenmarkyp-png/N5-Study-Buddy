...
import { getTaForm } from "@/lib/te-form";
import { useProgress } from "@/hooks/use-progress";
import { Clock, CheckCircle2, XCircle, ArrowRight, RotateCcw, Trophy, Info } from "lucide-react";
...
export default function QuizTaForm() {
  ...
  const verbPool = allWords.filter(w =>
    w.type.includes('verb') && w.verbGroup && getTaForm(w) !== null
  );
  ...
  const [questions, setQuestions] = useState<TaQuestion[]>([]);
  ...
  const startQuiz = () => {
    ...
    const allTaForms = verbPool
      .map(w => getTaForm(w))
      .filter(Boolean) as { kana: string; romaji: string; rule: string }[];

    const qs: TaQuestion[] = picked.map(word => {
      const tf = getTaForm(word)!;
      const wrongKanas = fisherYates(
        allTaForms.filter(t => t.kana !== tf.kana)
      )
        .slice(0, 3)
        .map(t => t.kana);
  ...
  if (!isStarted) {
    ...
    return (
      <div className="w-full max-w-lg mx-auto py-12 px-4 flex flex-col items-center justify-center min-h-[70vh] text-center space-y-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2">た-Form Quiz</h1>
          <p className="text-muted-foreground text-lg">Practice past tense verb conjugation.</p>
        </div>
        ...
        <div className="w-full bg-muted/40 border border-border rounded-2xl p-4 text-left space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground mb-2">Same pattern as て-form — just replace て→た and で→だ</p>
          {[
            ['う/つ/る (godan)', '→ った', 'kau → katta, matsu → matta'],
            ['む/ぶ/ぬ (godan)', '→ んだ', 'yomu → yonda'],
            ['く (godan)', '→ いた', 'kaku → kaita'],
            ['ぐ (godan)', '→ いだ', 'oyogu → oyoida'],
            ['す (godan)', '→ した', 'hanasu → hanashita'],
            ['る-verb (ichidan)', '→ た', 'taberu → tabeta'],
            ['する / compound', '→ した', 'benkyousuru → benkyoushita'],
            ['くる', '→ きた', 'kuru → kita'],
        ...
        >
          <div className="bg-card border-2 border-card-border rounded-3xl p-8 text-center mb-6 shadow-sm flex-1 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-sm font-bold uppercase tracking-wider text-muted-foreground">
              What is the た-form (past tense) of...
            </p>
